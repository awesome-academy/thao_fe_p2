...
export default function AdminBookingsClient({
  ...
  const formatDate = (date: Date | string) => {
    const dateInstance: Date = typeof date === 'string' ? new Date(date) : date;
    const localeString =
      locale === 'vi' ? LOCALE_STRINGS.VIETNAMESE : LOCALE_STRINGS.ENGLISH;
    return dateInstance.toLocaleDateString(localeString);
  };

  const formatPaymentStatus = (payment: BookingWithRelations['payment']) => {
  ...
  const getPaymentStatusColor = (payment: BookingWithRelations['payment']) => {
